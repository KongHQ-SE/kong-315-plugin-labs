# Alternate Lab 02 — Plugin Streaming (business-hours gate)

> **This is an alternate.** The main [Lab 02](../lab-02-plugin-streaming/) uses a
> JWT claims-to-headers plugin, which has a much stronger customer story. Use
> this version if you want simpler Lua, or a demo with no auth context.
>
> Same feature, same mechanics, same gotchas. Commands below assume you run it
> in place of the main Lab 02.
...
```bash
./bin/deck gateway sync alternate-business-hours/step1-plugin.yaml --include-plugin-definitions
```
...
```bash
./alternate-business-hours/verify.sh
```
...
```bash
./bin/deck gateway sync alternate-business-hours/step2-closed.yaml --include-plugin-definitions
```
...
```bash
./bin/deck gateway sync alternate-business-hours/step3-hotpatch.yaml --include-plugin-definitions
```
...
- **No `require()`** of your own modules (Kong's bundled libraries are fine —
  `cjson`, `cjson.safe`, `resty.string` and `os.date` were all verified working)
- **No `init_worker` phase, no timers**
- **No filesystem access**
- **Not supported on Serverless Gateways** (Dedicated Cloud and self-managed only)

This handler computes day-of-week with arithmetic on `ngx.time()` rather than
calling `os.date()`. `os.date()` does work — the reason to avoid it is that it
depends on the container's timezone data, which is a deployment variable you
don't control across a fleet.
